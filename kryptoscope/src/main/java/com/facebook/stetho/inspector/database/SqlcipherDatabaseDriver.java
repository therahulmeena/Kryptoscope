...
@ThreadSafe
public class SqlcipherDatabaseDriver
        extends DatabaseDriver2<SqlcipherDatabaseDriver.SqlcipherDatabaseDescriptor> {
    ...
    private final SQLCDatabaseConnectionProvider mDatabaseConnectionProvider;
     ...
     * @deprecated use {@link SqlcipherDatabaseDriver#SqliteDatabaseDriver(Context, String, DatabaseFilesProvider, DatabaseConnectionProvider)}
     */
    @Deprecated
    public SqlcipherDatabaseDriver(Context context, String sqlCipherDBKey) {
        this(
                context,
                new DefaultDatabaseFilesProvider(context),
                new DefaultSQLCDatabaseConnectionProvider(sqlCipherDBKey));
    }

    /**
     * @deprecated use {@link SqlcipherDatabaseDriver(Context, String, DatabaseFilesProvider, DatabaseConnectionProvider)}
     */
    @Deprecated
    public SqlcipherDatabaseDriver(
            Context context,
    ...
    public SqlcipherDatabaseDriver(
            Context context,
            DatabaseFilesProvider databaseFilesProvider,
            SQLCDatabaseConnectionProvider databaseConnectionProvider) {
        super(context);
    ...
    public List<SqlcipherDatabaseDescriptor> getDatabaseNames() {
        ArrayList<SqlcipherDatabaseDescriptor> databases = new ArrayList<>();
        ...
        for (File database : tidiedList) {
            databases.add(new SqlcipherDatabaseDescriptor(database));
        }
        return databases;
    }
    ...
    public Database.ExecuteSQLResponse executeSQL(
            SqlcipherDatabaseDescriptor databaseDesc,
            String query,
    ...
    static class SqlcipherDatabaseDescriptor implements DatabaseDescriptor {
        public final File file;

        public SqlcipherDatabaseDescriptor(File file) {
            this.file = file;
        }
