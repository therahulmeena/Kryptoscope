...
import com.facebook.stetho.inspector.database.DefaultSQLCDatabaseConnectionProvider;
import com.facebook.stetho.inspector.database.SqlcipherDatabaseDriver;
...
public class Stetho {
  ...
  public static void initializeWithDefaults(final Context context, final String sqlCipherDBKey) {
    initialize(new Initializer(context) {
      ...
      @Override
      protected Iterable<ChromeDevtoolsDomain> getInspectorModules() {
        return new DefaultInspectorModulesBuilder(context).provideDatabaseDriver(new SqlcipherDatabaseDriver(context,
                new DefaultDatabaseFilesProvider(context),
                new DefaultSQLCDatabaseConnectionProvider(sqlCipherDBKey))).excludeSqliteDatabaseDriver(true).finish();
      }
    });
  }
  /**
